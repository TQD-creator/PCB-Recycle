...
async def get_scan_status(task_id: str) -> TaskStatusResponse:
    ...
    if task.state in {"PENDING", "RECEIVED", "STARTED"}:
        return TaskStatusResponse(
            task_id=task_id, state=ScanState.processing if task.state != "PENDING" else ScanState.queued,
    ...
    if task.state == "PROGRESS":
        meta = task.info or {}
        return TaskStatusResponse(task_id=task_id, state=ScanState.processing, stage=str(meta.get("stage", "Processing")), progress=int(meta.get("progress", 10)))
    ...
    if task.state == "SUCCESS":
        ...
        return TaskStatusResponse(
            task_id=task_id, state=ScanState.completed, stage=str(payload.get("stage", "Completed")),
            progress=int(payload.get("progress", 100)), image_url=payload.get("image_url"),
            report=report, triage_queue=payload.get("triage_queue", []),
        )

    error_message = "Unknown processing error"
    if isinstance(task.info, dict): error_message = str(task.info.get("error", error_message))
    elif task.info is not None: error_message = str(task.info)

    return TaskStatusResponse(task_id=task_id, state=ScanState.failed, stage="Failed", progress=100, error=error_message)
...
async def resolve_anchor(request: ResolveAnchorRequest) -> ResolveAnchorResponse:
    try:
        # Dispatch to the Celery Worker so it updates its own RAM securely
        ...
        # Wait up to 10 seconds for the worker to update the DB and FAISS
